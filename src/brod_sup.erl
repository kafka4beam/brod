...
-spec start_client([brod:endpoint()], brod:brod_client_id(), brod:client_config()) ->
                      ok | {error, any()}.
...
-spec stop_client(brod:brod_client_id()) -> ok | {error, any()}.
stop_client(ClientId) ->
  _ = supervisor3:terminate_child(?SUP, ClientId),
  supervisor3:delete_child(?SUP, ClientId).

-spec find_client(brod:brod_client_id()) -> [pid()].
find_client(Client) ->
