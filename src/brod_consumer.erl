...
handle_fetch_response_test() ->
  State0 = #state{},
  PM0 = #partition_messages{error_code = offset_out_of_range},
  R0 = #fetch_response{topics = [#topic_fetch_data{partitions = [PM0]}]},
  ?assertEqual({error, offset_out_of_range},
               handle_fetch_response(R0, State0)),
  PM1 = #partition_messages{error_code = no_error, messages = []},
  ...
  PM2 = #partition_messages{error_code = no_error, messages = [foo], last_offset = 1},
