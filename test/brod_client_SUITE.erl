...
%%
%% The fix is for `brod_producers_sup:post_init/1' to read the partition
%% count directly from brod_client's `workers_tab' ETS cache (populated by
%% `validate_topic_existence/3' immediately before the per-topic supervisor
%% is started) instead of issuing a `gen_server:call' back into brod_client.
%% This test asserts that property white-box: after `brod:start_producer/3'
...
t_start_producer_does_not_post_init_callback(Config) when is_list(Config) ->
  ...
  ?assertEqual(
    ...
    "when the partition count cache is populated"),
  ok.
