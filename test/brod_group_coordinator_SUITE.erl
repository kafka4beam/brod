...
-export([ t_acks_during_revoke/1
        ...
        , t_fenced_static_member_retries_by_default/1
...
t_fenced_static_member_retries_by_default(Config) when is_list(Config) ->
  case kafka_test_helper:kafka_version() of
    Vsn when Vsn < {2, 3} ->
      {skip, "no static group membership"};
    _ ->
      GroupId = unique_group_id("static-fencing-retry"),
      GroupConfig =
